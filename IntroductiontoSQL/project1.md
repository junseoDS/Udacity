...
## which countries are top 10 

SELECT co.country, c.city, COUNT(rental_date)
FROM country co
...
SELECT co.country country, ca.name category, COUNT(*) count_category
FROM category ca
JOIN film_category fc ON ca.category_id= fc.category_id
JOIN film f ON f.film_id=fc.film_id
JOIN inventory i ON i.film_id=f.film_id
JOIN retal r ON i.inventory_id=r.inventory_id
...
나라최대
SELECT country, MAX(count_category)
FROM
(SELECT co.country country, ca.name category, COUNT(*) count_category
FROM category ca
JOIN film_category fc ON ca.category_id= fc.category_id
...
JOIN country co ON c.country_id=co.country_id
GROUP BY 1,2
ORDER BY 1,3 DESC) ccc
GROUP BY 1
ORDER BY 1

## Favorite categories of movies by country and ratio
WITH ccc AS (SELECT co.country country, ca.name category, COUNT(*) count_category
FROM category ca
JOIN film_category fc ON ca.category_id= fc.category_id
JOIN film f ON f.film_id=fc.film_id
JOIN inventory i ON i.film_id=f.film_id
...
FROM ccc
JOIN
(SELECT country, MAX(count_category) max_count
FROM ccc
GROUP BY 1
ORDER BY 1) c_max
ON c_max.country= ccc.country
AND c_max.max_count = ccc.count_category
...
ORDER BY 1,3 DESC),
total AS (
SELECT ccc.* , SUM(count_category) OVER (PARTITION BY country) total
FROM ccc
ORDER BY 1, 3 DESC)

SELECT total.* , total.count_category/total.total ratio
FROM total
JOIN 
(SELECT country, MAX(count_category) max_count
FROM ccc
